    ...
    def rate_code_spikes(self, neuron_count, output_spiketimes):
        ...
        for i in range(neuron_count):
            ...
            if i == 0:
                ...
                spike_bins_current += spikes_per_bin
        spike_bins_current = self.sliding_time_window(spike_bins_current,self.time_window) #Applies a time window to smooth the output        
        smoothed_spike_bins = gaussian_filter(spike_bins_current, netparams.convstd_rate) #Applies a filter to smooth the high frequency noise
